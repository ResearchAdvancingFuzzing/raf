...
# AFLGO variables
t_x = 10 # exploitation counter basically
max_distance = -1
min_distance = -1
cur_distance = -1
...
def calibrate_case(kbs, entry, queue_cycle, target):
    ...
    output_file = "out"
    cmd = "/aflgo/afl-showmap -o %s  -- %s %s" % (output_file, target, entry.filepath)
    cmd = cmd.split()
...
def calculate_score(cfg, kbs, entry, avg_exec_time, avg_bitmap_size, fuzz_mu): 
    ...
    ##########################################################
    ##                    WARNING                           ##
    ##          below changes are untested                  ##
    ##########################################################

    # my understanding since we don't have pointer access like in C
    cur_distance = entry.depth
    if (cur_distance > 0):
    ...
    # linear cooling calculation

    cur_ms = time.time()

    t = (cur_ms - start_time) / 1000.0

    progress_to_tx = (t / t_x * 60.0)
    ...
    if (cur_distance > 0): # beleive this is the current distance, but the original source code is a bit different
        nomalized_d = 0
        if (max_distance != min_distance):
                normalized_d = (distance - min_distance) / (max_distance - min_distance)
        if(normalized_d >= 0):
             p = (1.0 - normalized_d) * (1.0 - T) + 0.5 * T
             power_factor = pow(2.0, 2.0 * math.log2(32.0) * (p - 0.5)) # 32 == MAX_FACTOR from source
