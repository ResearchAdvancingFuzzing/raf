...
spitfire_taint_protobuf = "spitfire_taint.pb"

spitire_protobuf_out sys.argv[2]
...
with open(panda_protobuf, "rb") as pbf:
    ...
    while True:
        try:
            ...
            if log_entry.HasField("asid_info"):
                ...
                if ai.name is the_program:
                    asids.add(ai.asid)
                    instr_interval = [ai.start_instr, ai.end_instr]
                    instr_intervals.add(instr_interval)
...
print "Found %d tainting fbs" % (len(tainting_fbs))
...
for fbs in tainting_fbs.keys():
    ...
    for tiv in tainting_fbs[fbs]:
        ...
        tm = TaintMapping(f, i, None, tiv.len, (float(tiv.instr - first_instr_for_prog) / (last_instr_for_prog - first_instr_for_prog)), tiv.tcn_min, tiv.tcn_max)
        ta.add_taint_mapping(tm)

with open(spitfire_protobuf_out, "w") as s:
    s.write(ta.to_protobuf())
